...
public final class IntentsActionPolicy extends ActionsPolicy {
        ...
        try {
            ...
            if (choose) {
                ...
                if (infos.size() > 0) {
                    ...
                    if (!handledByEditorInManifest(ctx, fso)) {
                ...
                resolveIntent(ctx, intent, onDismissListener);
            }
        } catch (Exception e) {
    ...
    private static boolean handledByEditorInManifest(Context context, FileSystemObject fso) {
        MimeTypeCategory category = MimeTypeHelper.getCategory(context, fso);
        return (fso instanceof RegularFile &&
    ...
    private static void resolveIntent(
            Context ctx, Intent intent, OnDismissListener onDismissListener) {
        try {
