...
public final class BookmarksActionPolicy extends ActionsPolicy {
	...
	public static Bookmark addToBookmarks(final Context ctx, final FileSystemObject fso) {
		try {
			// Create the bookmark
			Bookmark bookmark = new Bookmark(BOOKMARK_TYPE.USER_DEFINED,
					fso.getName(), fso.getFullPath());
			bookmark = Bookmarks.addBookmark(ctx, bookmark);
			if (bookmark == null) {
				// The operation fails
				DialogHelper.showToast(ctx, R.string.msgs_operation_failure,
						Toast.LENGTH_SHORT);
				return null;
			}

			// Success
			DialogHelper.showToast(ctx, R.string.bookmarks_msgs_add_success,
					Toast.LENGTH_SHORT);
			return bookmark;
