...
public class SearchActivity extends Activity
        ...
        try {
            ...
            if (fileCategory == MimeTypeCategory.VIDEO) {
                ...
                for (FileSystemObject f : filteredList) {
                    uris.add(Uri.parse("file://" + f.getFullPath()));
                }

                Intent intent = new Intent();
                intent.setAction(Intent.ACTION_VIEW);
                intent.setDataAndType(Uri.parse("file://" + fso.getFullPath()),
                        MimeTypeHelper.getMimeType(this, fso) );
    ...
    private ArrayList<FileSystemObject> filterSearchResults(MimeTypeCategory category) {
        if (mAdapter.getCount() < 1) return null;
